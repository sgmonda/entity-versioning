...
Deno.test("MySQL introspect - getTables returns all expected tables", async () => {
  await setup();
  try {
...
Deno.test("MySQL introspect - getTables detects column info correctly", async () => {
  await setup();
  try {
    const tables = await connector.getTables();
...
Deno.test("MySQL introspect - getTables detects PKs correctly", async () => {
  await setup();
  try {
    const tables = await connector.getTables();
...
Deno.test("MySQL introspect - getForeignKeys returns all FK relationships", async () => {
  await setup();
  try {
...
Deno.test("MySQL introspect - tables without PK", async () => {
  await setup();
  try {
    const tables = await connector.getTables();
...
Deno.test("MySQL introspect - tables without FKs", async () => {
  await setup();
  try {
    const fks = await connector.getForeignKeys();
